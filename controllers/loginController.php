...
class loginController extends Controller {
	...
	public function add(){
		...
		if(!in_array("",$dados_form)):
			if($dados_form['v_passwd'] != $dados_form['user_passwd']):
				$dados['return'] = $this->ajaxSuccess("Senhas não conferem, favor digitar novamente.");
			else:
				unset($dados_form['v_passwd']);
				$dados_form['user_passwd'] = md5($dados_form['user_passwd']);
				// Fazer algo
				if(Helpers::isMail($dados_form['user_email'])):
...
// TODO: Função para login do sistema.
	public function login(){
	  ...
	  if(!in_array("",$dados_form)):
				if(Helpers::isMail($dados_form['user_email'])):
		    $return = Users::doLogin($dados_form['user_email'],$dados_form['user_passwd']);
		    if($return === 1):
		      $dados['return'] = $this->ajaxWarning('<a href="'.BASE.'/login/cadastro">clique aqui para cadastrar</a>.','E-mail não encontrado.');
		    elseif($return === 2):
